...
$app->get("/testing", function() use($app) {
    echo "Pruebas con Api Rest (rutas)";
});
...
$app->get('/delete-producto/:id', function($id) use($db,$app) {
    ...
    if($query){
        ...
        $result = array(
            'status' => 'error',
            'code' => 404,
            'message' => 'No se pudo elimir el producto'
        );
    }
    echo json_encode($result);
...
$app->post('/update-producto/:id', function($id) use($db, $app){
    ...
    if($query){
        ...
        $result = array(
            'status' => 'error',
            'code' => 404,
            'message' => 'El producto NO se ha actulizado correctamente'
        );
    }
    echo json_encode($result);
});
...
$app->post('/upload-file', function() use ($db,$app) {
    ...
    if(isset($_FILES['uploads'])){
        ...
        $upload = $piramideUploader->upload('image', 'uploads', 'uploads', array('image/jpeg', 'image/pgn', 'image/gif'));
...
$app->post('/productos', function() use($app,$db){
    ...
    $result = array(
        'status' => 'error',
        'code' => 404,
        'message' => 'Producto NO SE HAinsertado correctamente'
    );
